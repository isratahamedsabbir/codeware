...
class Index extends Component
{
    ...
    /**
     * Per-section status used by the overview grid at the top of the page.
     * Each section maps to the tab that hosts it so a tile click can jump
     * straight to the right panel. State is one of 'configured' (all its
     * keys are set), 'partial' (some are) or 'empty' (none are).
     *
     * @return array<string, array{title: string, icon: string, tab: string, state: string, note: string}>
     */
    public function sectionStatuses(): array
    {
        $set = fn (string $key): bool => filled($this->env[$key] ?? null);

        $statusOf = function (array $keys) use ($set): string {
            $present = collect($keys)->filter(fn (string $key) => $set($key))->count();

            return $present === count($keys) ? 'configured' : ($present > 0 ? 'partial' : 'empty');
        };
        ...
        return [
            'app' => [
                ...
                'icon' => 'rocket-launch',
                'tab' => 'general',
                'state' => $statusOf(['APP_NAME', 'APP_URL']),
                'note' => 'Identity, URLs & cache store',
            ],
            'google-login' => [
                'title' => 'Google Login',
                'icon' => 'globe-alt',
                'tab' => 'authentication',
                'state' => $statusOf(['GOOGLE_CLIENT_ID', 'GOOGLE_CLIENT_SECRET']),
                'note' => 'Sign in with a Google account',
            ],
            'facebook-login' => [
                'title' => 'Facebook Login',
                'icon' => 'chat-bubble-left-right',
                'tab' => 'authentication',
                'state' => $statusOf(['FACEBOOK_CLIENT_ID', 'FACEBOOK_CLIENT_SECRET']),
                'note' => 'Sign in with a Facebook account',
            ],
            'recaptcha' => [
                'title' => 'reCAPTCHA',
                'icon' => 'shield-check',
                'tab' => 'authentication',
                'state' => $statusOf(['RECAPTCHA_SITE_KEY', 'RECAPTCHA_SECRET_KEY']),
                'note' => 'Bot protection on the login form',
            ],
            'pixel' => [
                'title' => 'Google Pixel',
                'icon' => 'chart-bar',
                'tab' => 'integrations',
                'state' => filled($this->settings['google_pixel_id'] ?? null) ? 'configured' : 'empty',
                'note' => 'Analytics & Ads tracking ID',
            ],
            'google-maps' => [
                'title' => 'Google Maps',
                'icon' => 'map',
                'tab' => 'integrations',
                'state' => $set('GOOGLE_MAPS_API_KEY') ? 'configured' : 'empty',
            ...
            'aws-s3' => [
                ...
                'icon' => 'cloud',
                'tab' => 'integrations',
                'state' => $statusOf(['AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY', 'AWS_BUCKET']),
                'note' => 'Cloud file storage',
            ],
            'firebase' => [
                'title' => 'Firebase',
                'icon' => 'fire',
                'tab' => 'integrations',
                'state' => filled($this->env['FIREBASE_CREDENTIALS_PATH'] ?? null)
                    ? ($this->firebaseCredentialsExist() ? 'configured' : 'partial')
                    : 'empty',
            ...
            'cms-editor' => [
                ...
                'icon' => 'pencil-square',
                'tab' => 'integrations',
                'state' => $set('CMS_EDITOR_BASE_URL') ? 'configured' : 'empty',
                'note' => 'Visual editor for pages & products',
            ],
        ];
    }
