...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        @php
            $portalPrimaryColor = \App\Models\Setting::get('primary_color', '#1e7bc4');
            $portalSecondaryColor = \App\Models\Setting::get('secondary_color', '#7cc242');
        @endphp
        <style>
            ...
            :root {
                --color-primary: {{ $portalPrimaryColor }};
                --color-secondary: {{ $portalSecondaryColor }};
                --color-accent: {{ $portalPrimaryColor }};
                --color-accent-content: {{ $portalPrimaryColor }};
            }
        </style>
    </head>
    <body class="min-h-screen bg-slate-50 antialiased dark:bg-zinc-900">
        ...
        <flux:sidebar sticky collapsible="mobile" class="admin-sidebar"
            ...
            {{-- Nav — same .admin-nav-item classes and active/hover treatment as the
                 ...
                 collapsible groups since the delivery portal's nav is flat. --}}
            <nav class="flex-1 overflow-y-auto px-2 py-2">
                <div class="px-3 py-2 text-sm text-zinc-400 font-medium leading-none">{{ __('Delivery Portal') }}</div>
                ...
                <div x-show="search && {{ \Illuminate\Support\Js::from($portalNavItems->pluck('label')->values()) }}.every(l => !matches(l))"
            ...
            <form method="POST" action="{{ route('delivery.logout') }}" class="hidden lg:block px-2 pb-2 shrink-0">
                @csrf
                <button type="submit"
            ...
            <div class="flex-1 p-3 max-w-[1600px] w-full mx-auto">
                @unless ($hideHeading ?? false)
                    <div class="mb-5 flex items-center justify-between gap-4 flex-wrap">
                        <flux:heading size="xl">{{ $title ?? 'Delivery Portal' }}</flux:heading>
            ...
            <footer class="shrink-0 border-t border-zinc-200/70 bg-white/60 backdrop-blur px-4 py-4 md:px-6 dark:border-zinc-700/70 dark:bg-zinc-900/60">
                <div class="max-w-[1600px] w-full mx-auto flex flex-col sm:flex-row items-center justify-between gap-2 text-xs font-medium text-zinc-400">
                    ...
                    <p class="flex items-center gap-1.5 text-zinc-400">
                        <flux:icon.truck class="size-3.5" />
                        Delivery Portal
                    </p>
                </div>
            </footer>
