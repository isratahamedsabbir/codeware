...
<main class="mx-auto max-w-7xl px-4 py-8 sm:px-6">
    <header class="mb-8">
        <h1 class="text-3xl font-extrabold text-sf-heading sm:text-4xl">{{ $category->name }}</h1>
        ...
        <div class="mb-8 flex flex-wrap gap-3">
            @foreach ($children as $child)
                <a href="{{ route('shop.category', $child->slug) }}"
                    class="rounded-full border border-zinc-200 px-4 py-2 text-sm font-medium text-zinc-700 transition hover:border-brand hover:text-brand">
                    {{ $child->name }}
                </a>
            @endforeach
        </div>
    ...
    <section>
        <div class="mb-6 flex flex-wrap items-end justify-between gap-x-4 gap-y-2">
            <div>
                <h2 class="text-2xl font-bold text-sf-heading">{{ __('Products in :category', ['category' => $category->name]) }}</h2>
        ...
        @else
            <div class="flex flex-col items-center justify-center rounded-card border border-dashed border-zinc-200 py-24 text-center">
                <h2 class="text-lg font-semibold text-sf-heading">{{ __('No products in this category yet') }}</h2>
