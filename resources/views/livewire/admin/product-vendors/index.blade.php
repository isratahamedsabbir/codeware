...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($productVendors as $vendor)
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors" @contextmenu.prevent="$el.querySelector('[data-actions-trigger]')?.click()">
                            ...
                            <td class="px-4 py-2">
                                @if ($vendor->logo)
                                    <img src="{{ $vendor->logo }}" alt="{{ $vendor->name }}" class="h-8 w-8 rounded-md object-cover border border-zinc-200" />
                                @else
                            ...
                            <td class="px-4 py-2">
                                @if ($vendor->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $vendor->id }})"
    ...
    {{-- Delete Modal --}}
    <flux:modal name="product-vendor-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'product-vendor-delete') $flux.modal('product-vendor-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'product-vendor-delete') $flux.modal('product-vendor-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete vendor?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">
                This action cannot be undone. Products already assigned to this vendor keep their product record — they just lose the vendor association.
            </flux:text>
