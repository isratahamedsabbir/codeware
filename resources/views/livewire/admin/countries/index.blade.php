...
<div>
    ...
    <div class="mb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
            @include('partials.admin-breadcrumbs', ['routeName' => 'admin.countries'])
        </div>
        <div class="flex items-center gap-2 shrink-0">
            @if (count($selectedIds) > 0)
                ...
                <flux:button variant="outline" size="sm" icon="arrow-down-tray"
                    href="{{ route('admin.countries.export', ['ids' => $selectedIds]) }}">
            ...
            <div class="page-header-actions flex items-center gap-2 shrink-0">
                <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.countries.create') }}" wire:navigate>
                    New country
                </flux:button>
            </div>
        </div>
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
    ...
    <div class="flex items-center gap-3 p-4 flex-wrap">
        ...
        <div class="relative max-w-xs ml-auto">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search countries…"
            ...
            <table class="w-full divide-y divide-gray-200" style="table-layout:fixed">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($countries as $country)
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors cursor-default {{ in_array($country->id, $selectedIds, true) ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-300' : '' }}"
                            ...
                            <td class="px-4 py-2">
                                @if ($country->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $country->id }})"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-100 border-l border-zinc-100 px-4 py-2">
                                ...
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.countries.edit', $country->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary', 'disabled' => $bulkActive],
                                    ['wireClick' => 'confirmDelete(' . $country->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => $bulkActive],
                                ]" />
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm text-zinc-600">No countries found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="country-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'country-delete') $flux.modal('country-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'country-delete') $flux.modal('country-delete').close()">
        <div class="space-y-4">
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="country-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'country-bulk-delete') $flux.modal('country-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'country-bulk-delete') $flux.modal('country-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ \Illuminate\Support\Str::plural('country', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This will also permanently delete all divisions, districts and
                upazilas under these countries.
            </flux:text>
            <div class="flex gap-2 pt-1">
