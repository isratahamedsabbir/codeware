<div class="max-w-[1600px] w-full mx-auto flex-1">
    ...
    @push('page-header-actions')
        <flux:button variant="ghost" size="sm" class="admin-back-btn" icon="arrow-left" href="{{ route('admin.upazilas') }}" wire:navigate>
            Back
        </flux:button>
    @endpush
    ...
    <div class="w-full max-w-2xl bg-white rounded-[5px] shadow-sm p-6 space-y-4">
        ...
        <flux:field>
            ...
            <flux:select wire:model="districtId">
                <flux:select.option value="">— Select district —</flux:select.option>
                @foreach ($districts as $district)
                    <flux:select.option value="{{ $district->id }}">{{ $district->name }} — {{ $district->division->name }}</flux:select.option>
                @endforeach
            </flux:select>
        ...
        <flux:field>
            ...
            <flux:input wire:model="name" placeholder="e.g. Savar" />
        ...
        <div class="-mx-6 -mb-6 mt-6 flex items-center gap-3 flex-wrap rounded-b-lg border-t border-zinc-100 bg-zinc-50/60 px-6 py-4 dark:border-zinc-700 dark:bg-zinc-800/40">
            <button wire:click="save" wire:loading.attr="disabled" wire:target="save"
                ...
                <span wire:loading.remove wire:target="save">{{ $upazilaId ? 'Update Upazila' : 'Create Upazila' }}</span>
