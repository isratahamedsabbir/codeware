<div class="bg-white rounded-lg border border-zinc-100 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between gap-3 px-6 py-5 border-b border-zinc-100 flex-wrap">
        <div>
            <h1 class="text-lg font-semibold text-zinc-900">Upazilas</h1>
            <p class="text-sm text-zinc-600 mt-0.5">Manage Bangladesh upazilas (sub-districts)</p>
        </div>
        <a href="{{ route('admin.upazilas.create') }}" wire:navigate
    ...
    <div class="flex gap-3 px-6 py-3 border-b border-zinc-100 flex-wrap">
        ...
        <select wire:model.live="districtFilter"
            ...
            @foreach ($this->districts as $district)
                <option value="{{ $district->id }}">{{ $district->getTranslation('name', 'en', false) }}</option>
            @endforeach
        </select>
                        ...
                        <tr class="border-b border-zinc-50 hover:bg-indigo-50/30 transition-colors">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="font-medium text-zinc-900 text-sm leading-snug">
                                    {{ $upazila->getTranslation('name', 'en', false) }}
                                </div>
                                @if ($upazila->getTranslation('name', 'bn', false))
                                    <div class="text-xs text-zinc-600 mt-0.5">
                                        {{ $upazila->getTranslation('name', 'bn', false) }}
                                    </div>
                                @endif
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                @if ($upazila->district)
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-zinc-100 text-zinc-500">
                                        {{ $upazila->district->getTranslation('name', 'en', false) }}
                                    </span>
                                @else
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <div class="relative group">
                                        <a href="{{ route('admin.upazilas.edit', $upazila->id) }}" wire:navigate
                                            aria-label="Edit upazila"
                                    ...
                                    {{-- Delete --}}
                                    <div class="relative group">
                                        <button wire:click="confirmDelete({{ $upazila->id }})"
                                            aria-label="Delete upazila"
    ...
    <flux:modal name="upazila-delete" class="md:w-80"
        ...
        <div class="space-y-4">
            ...
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The upazila will be soft-deleted.
            </flux:text>
