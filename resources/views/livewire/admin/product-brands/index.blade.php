...
<div>
    ...
    <div class="mb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
            @include('partials.admin-breadcrumbs', ['routeName' => 'admin.product-brands'])
        </div>
        <div class="flex items-center gap-2 shrink-0">
            @if (count($selectedIds) > 0)
                ...
                <flux:button variant="outline" size="sm" icon="arrow-down-tray"
                    href="{{ route('admin.product-brands.export', ['ids' => $selectedIds]) }}">
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
            ...
            <table class="w-full divide-y divide-gray-200" style="table-layout:fixed">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($productBrands as $brand)
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors cursor-default {{ in_array($brand->id, $selectedIds, true) ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-300' : '' }}"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-100 border-l border-zinc-100 px-4 py-2">
                                ...
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.product-brands.edit', $brand->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary', 'disabled' => $bulkActive],
                                    ['wireClick' => 'confirmDelete(' . $brand->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => $bulkActive],
                                ]" />
                            </td>
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="product-brand-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'product-brand-bulk-delete') $flux.modal('product-brand-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'product-brand-bulk-delete') $flux.modal('product-brand-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ \Illuminate\Support\Str::plural('brand', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">
                This action cannot be undone. Products already assigned to these brands keep their product record — they just lose the brand association.
            </flux:text>
            <div class="flex gap-2 pt-1">
