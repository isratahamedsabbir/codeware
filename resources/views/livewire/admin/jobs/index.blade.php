<div class="bg-white rounded-lg border border-zinc-100 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between gap-3 px-6 py-5 border-b border-zinc-100 flex-wrap">
        <div>
            <h1 class="text-lg font-semibold text-zinc-900">Jobs</h1>
            <p class="text-sm text-zinc-600 mt-0.5">Manage career job listings. Drag to reorder.</p>
        </div>
        <a href="{{ route('admin.jobs.create') }}" wire:navigate
        ...
        x-data="{
            init() {
                ...
                new Sortable(this.$refs.sortableRows, {
                    ...
                    onEnd: (evt) => {
                        const rows = [...this.$refs.sortableRows.querySelectorAll('[data-job-id]')];
                        const order = rows.map(r => parseInt(r.dataset.jobId));
                        $wire.reorder(order);
                    }
                });
            }
            ...
            <table class="w-full border-collapse" style="table-layout:fixed">
                ...
                <thead>
                    <tr class="bg-zinc-50 border-b border-zinc-100">
                        ...
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Department</th>
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Status</th>
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Vacancy</th>
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Deadline</th>
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Applications</th>
                ...
                <tbody x-ref="sortableRows">
                    @forelse ($jobs as $job)
                        <tr class="border-b border-zinc-50 hover:bg-indigo-50/30 transition-colors" data-job-id="{{ $job->id }}">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="font-medium text-zinc-900 text-sm leading-snug">
                                    {{ $job->getTranslation('title', 'en', false) }}
                                </div>
                                @if ($job->getTranslation('title', 'bn', false))
                                    <div class="text-xs text-zinc-600 mt-0.5">
                                        {{ $job->getTranslation('title', 'bn', false) }}
                                    </div>
                                @endif
                            </td>
                            ...
                            {{-- Slug --}}
                            <td class="px-4 py-3.5">
                                <span class="text-sm text-zinc-500 font-mono">{{ $job->slug }}</span>
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                @if ($job->department)
                                    <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-zinc-100 text-zinc-500">
                                        {{ $job->department->getTranslation('name', 'en', false) }}
                                    </span>
                                @else
                            ...
                            {{-- Vacancy --}}
                            <td class="px-4 py-3.5">
                                <span class="text-sm text-zinc-600">{{ $job->vacancy }}</span>
                            </td>

                            {{-- Deadline --}}
                            <td class="px-4 py-3.5">
                                <span class="text-sm text-zinc-600">{{ $job->deadline?->format('d M Y') ?? '—' }}</span>
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <div class="relative group">
                                        <a href="{{ route('admin.jobs.edit', $job->id) }}" wire:navigate
                                            aria-label="Edit job"
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm text-zinc-600">No jobs found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="job-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'job-delete') $flux.modal('job-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'job-delete') $flux.modal('job-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete job?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The job will be soft-deleted.
            </flux:text>
