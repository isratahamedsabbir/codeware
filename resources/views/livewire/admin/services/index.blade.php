...
<div>
    ...
    <div class="mb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
            @include('partials.admin-breadcrumbs', ['routeName' => 'admin.services'])
        </div>
        <div class="flex items-center gap-2 shrink-0">
            @if (count($selectedIds) > 0)
                ...
                <flux:button variant="outline" size="sm" icon="arrow-down-tray"
                    href="{{ route('admin.services.export', ['ids' => $selectedIds]) }}">
            ...
            <div class="page-header-actions flex items-center gap-2 shrink-0">
                <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.services.create') }}" wire:navigate>
                    New service
                </flux:button>
            </div>
        </div>
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
            ...
            <table class="w-full divide-y divide-gray-200" style="table-layout:fixed">
                ...
                <thead>
                    <tr class="bg-zinc-50">
                        ...
                        <th class="px-2 py-2.5 text-center text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider w-8">#</th>
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Image</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($services as $service)
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors cursor-default {{ in_array($service->id, $selectedIds, true) ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-300' : '' }}"
                            @contextmenu.prevent="$el.querySelector('[data-actions-trigger]')?.click()"
                            @click="if ($event.ctrlKey || $event.metaKey) { $event.preventDefault(); $wire.toggleSelect({{ $service->id }}) }">
                            ...
                            <td class="px-2 py-2 text-center" @click.stop>
                                @if (count($selectedIds) > 0)
                                    <input type="checkbox" wire:click="toggleSelect({{ $service->id }})"
                            ...
                            <td class="px-2 py-2 text-center text-xs text-zinc-500">
                                <x-copy-text :text="$service->id" class="text-xs text-zinc-500">{{ $service->id }}</x-copy-text>
                            </td>

                            {{-- Image --}}
                            <td class="px-4 py-2">
                                @if ($service->featured_image)
                                    <img src="{{ $service->featured_image }}" alt="{{ $service->getTranslation('name', 'en', false) }}" class="h-8 w-8 rounded-md object-cover border border-zinc-200" />
                                @else
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-50/30 border-l border-zinc-100 px-4 py-2">
                                ...
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.services.edit', $service->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary', 'disabled' => $bulkActive],
                                    ['wireClick' => 'confirmDelete(' . $service->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => $bulkActive],
                                ]" />
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                <flux:icon.wrench-screwdriver class="w-10 h-10 text-zinc-200 mx-auto mb-3" />
                                <p class="text-sm text-zinc-600">No services found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="service-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'service-delete') $flux.modal('service-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'service-delete') $flux.modal('service-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete service?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The service will be soft-deleted.
            </flux:text>
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="service-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'service-bulk-delete') $flux.modal('service-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'service-bulk-delete') $flux.modal('service-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ \Illuminate\Support\Str::plural('service', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The selected services will be soft-deleted.
            </flux:text>
            <div class="flex gap-2 pt-1">
