<div class="bg-white rounded-lg border border-zinc-100 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between gap-3 px-6 py-5 border-b border-zinc-100 flex-wrap">
        <div>
            <h1 class="text-lg font-semibold text-zinc-900">Dealers</h1>
            <p class="text-sm text-zinc-600 mt-0.5">Manage Bangladesh dealer network. Drag to reorder.</p>
        </div>
        <a href="{{ route('admin.dealers.create') }}" wire:navigate
    ...
    <div class="flex gap-3 px-6 py-3 border-b border-zinc-100 flex-wrap">
        <div class="relative flex-1 min-w-[180px]">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search dealers…"
        ...
        x-data="{
            init() {
                ...
                new Sortable(this.$refs.sortableRows, {
                    ...
                    onEnd: (evt) => {
                        const rows = [...this.$refs.sortableRows.querySelectorAll('[data-dealer-id]')];
                        const order = rows.map(r => parseInt(r.dataset.dealerId));
                        $wire.reorder(order);
                    }
                });
            }
            ...
            <table class="w-full border-collapse" style="table-layout:fixed">
                ...
                <thead>
                    <tr class="bg-zinc-50 border-b border-zinc-100">
                        ...
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Upazila</th>
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Categories</th>
                ...
                <tbody x-ref="sortableRows">
                    @forelse ($dealers as $dealer)
                        <tr class="border-b border-zinc-50 hover:bg-indigo-50/30 transition-colors" data-dealer-id="{{ $dealer->id }}">
                            ...
                            {{-- Slug --}}
                            <td class="px-4 py-3.5">
                                <span class="text-sm text-zinc-500 font-mono">{{ $dealer->slug }}</span>
                            </td>

                            {{-- District --}}
                            <td class="px-4 py-3.5">
                                <span class="text-sm text-zinc-600">{{ $dealer->district }}</span>
                            </td>

                            {{-- Upazila --}}
                            <td class="px-4 py-3.5">
                                <span class="text-sm text-zinc-600">{{ $dealer->upazila ?? '—' }}</span>
                            </td>

                            {{-- Categories count --}}
                            <td class="px-4 py-3.5">
                                <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-zinc-100 text-zinc-500">
                                    {{ $dealer->categories_count }}
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <div class="relative group">
                                        <a href="{{ route('admin.dealers.edit', $dealer->id) }}" wire:navigate
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm text-zinc-600">No dealers found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <flux:modal name="dealer-delete" class="md:w-80"
        ...
        <div class="space-y-4">
            ...
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The dealer will be soft-deleted.
            </flux:text>
