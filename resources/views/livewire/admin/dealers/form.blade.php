<div class="max-w-[1600px] w-full mx-auto flex-1">
    ...
    <flux:button variant="ghost" icon="arrow-left" href="{{ route('admin.dealers') }}" wire:navigate class="mb-4 border-2">
        Back
    </flux:button>
    ...
    <div class="flex gap-5 items-start">
        ...
        <div class="flex-1 min-w-0 bg-white rounded-lg border border-zinc-100 shadow-sm p-6">
            ...
            <h1 class="text-lg font-semibold text-zinc-900 mb-5 pb-4 border-b border-zinc-100">
                {{ $dealerId ? 'Edit Dealer' : 'New Dealer' }}
            </h1>
            ...
            <div x-data="{ locale: 'en' }">
                ...
                <div x-show="locale==='en'" class="space-y-4">
                    ...
                    <flux:field>
                        ...
                        <flux:textarea wire:model="address_en" rows="2" placeholder="Full address in English…" />
                    </flux:field>
                </div>
                ...
                <div x-show="locale==='bn'" class="space-y-4">
                    <flux:field>
                        <flux:label>নাম <span class="text-red-500 ml-0.5">*</span></flux:label>
                    ...
                    <flux:field>
                        <flux:label>ঠিকানা</flux:label>
                        <flux:textarea wire:model="address_bn" rows="2" placeholder="বাংলায় সম্পূর্ণ ঠিকানা…" />
                    </flux:field>
                </div>
                ...
                <div class="mt-5 pt-4 border-t border-zinc-100">
                    <div class="grid grid-cols-2 gap-4">
                        <flux:field>
                            ...
                            <flux:select wire:model.live="selectedDistrictId">
                                <flux:select.option value="">— Select District —</flux:select.option>
                                @foreach ($this->districts as $district)
                                    <flux:select.option value="{{ $district->id }}">
                                        {{ $district->getTranslation('name', 'en', false) }}</flux:select.option>
                                @endforeach
                            </flux:select>
                        ...
                        <flux:field>
                            ...
                            <flux:select wire:model="upazila">
                                <flux:select.option value="">— Select Upazila —</flux:select.option>
                                @foreach ($this->upazilas as $upazila)
                                    <flux:select.option value="{{ $upazila->getTranslation('name', 'en', false) }}">
                                        {{ $upazila->getTranslation('name', 'en', false) }}</flux:select.option>
                                @endforeach
                            </flux:select>
                        </flux:field>
                    </div>
                ...
                <div class="mt-5 pt-4 border-t border-zinc-100">
                    <div class="grid grid-cols-2 gap-4">
                        <flux:field>
                            <flux:label>Phone <span class="text-red-500 ml-0.5">*</span></flux:label>
                ...
                {{-- Product Categories --}}
                <div class="mt-5 pt-4 border-t border-zinc-100">
                    <flux:label class="mb-3 block text-sm font-medium text-zinc-700">Product Categories</flux:label>
                    <div class="flex flex-wrap gap-3">
                        @foreach ($this->productCategories as $cat)
                            <label
                                class="flex items-center gap-2 text-sm text-zinc-600 px-3 py-2 rounded-md border border-zinc-200 bg-zinc-50 hover:border-indigo-400 hover:bg-indigo-50 hover:text-indigo-600 cursor-pointer transition-all">
                                <input type="checkbox" wire:model="selectedCategories" value="{{ $cat->id }}"
                                    class="w-3.5 h-3.5 accent-indigo-600 rounded border-zinc-300" />
        ...
        <div class="w-[320px] shrink-0 space-y-4">
            ...
            <div class="flex justify-end items-center gap-3 border-t border-zinc-100 flex-wrap pt-4">
                ...
                <button wire:click="save" wire:loading.attr="disabled"
                    ...
                    {{ $dealerId ? 'Update Dealer' : 'Create Dealer' }}
                </button>
            </div>
