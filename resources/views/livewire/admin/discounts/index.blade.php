<div>
    ...
    <div class="mb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
            @include('partials.admin-breadcrumbs', ['routeName' => 'admin.discounts'])
        </div>
        <div class="flex items-center gap-2 shrink-0">
            ...
            <div class="page-header-actions flex items-center gap-2 shrink-0">
                <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.discounts.create') }}" wire:navigate>
                    New discount
                </flux:button>
            </div>
        </div>
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
    ...
    <div class="flex items-center gap-3 p-4 flex-wrap">
        ...
        <div class="relative max-w-xs ml-auto">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search discounts…"
            ...
            <table class="w-full divide-y divide-gray-200">
                ...
                <thead>
                    <tr class="bg-zinc-50">
                        ...
                        <th class="hidden lg:table-cell px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Valid</th>
                        <th class="hidden lg:table-cell px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Starts</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($discounts as $discount)
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors {{ in_array($discount->id, $selectedIds, true) ? 'bg-indigo-50/50' : '' }}"
                            @contextmenu.prevent="$el.querySelector('[data-actions-trigger]')?.click()"
                            @click="if ($event.ctrlKey || $event.metaKey) { $event.preventDefault(); $wire.toggleSelect({{ $discount->id }}) }">

                            {{-- Expand toggle, small screens only, where columns are hidden. --}}
                            <td class="px-1 py-2 text-center">
                                <x-admin-row-expand-toggle class="lg:hidden" :expanded="$viewingId === $discount->id"
                                    wire:click="{{ $viewingId === $discount->id ? 'closeDetails' : 'viewDetails('.$discount->id.')' }}" />
                            </td>

                            {{-- Bulk-select checkbox --}}
                            <td class="px-2 py-2 text-center">
                                @if (count($selectedIds) > 0)
                                    <input type="checkbox" wire:click.stop="toggleSelect({{ $discount->id }})"
                                        @checked(in_array($discount->id, $selectedIds, true))
                            ...
                            <td class="hidden lg:table-cell px-2 py-2 text-center text-xs text-zinc-500">
                                <x-copy-text :text="$discount->id" class="text-xs text-zinc-500">{{ $discount->id }}</x-copy-text>
                            </td>

                            {{-- Name --}}
                            <td class="px-4 py-2">
                                <span class="text-sm font-semibold text-zinc-900"><x-truncate :text="$discount->name" /></span>
                            </td>
                            ...
                            <td class="px-4 py-2 text-sm text-zinc-700">
                                ...
                                @else
                                    {{ \App\Models\Setting::get('currency_symbol', '৳') }}{{ number_format((float) $discount->value, 2) }} off
                                @endif
                            </td>

                            {{-- Products --}}
                            <td class="hidden lg:table-cell px-4 py-2 text-xs">
                                @if ($discount->products_count > 0)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 border border-indigo-200">
                                        {{ $discount->products_count }} {{ Str::plural('product', $discount->products_count) }}
                                    </span>
                                @else
                            ...
                            <td class="hidden lg:table-cell px-4 py-2 text-xs">
                                ...
                                @elseif ($discount->ends_at && $discount->ends_at->isPast())
                            ...
                            <td class="hidden lg:table-cell px-4 py-2 text-xs text-zinc-500">
                                {{ $discount->starts_at?->toDisplay() ?: '—' }}
                            </td>

                            {{-- Status --}}
                            <td class="px-4 py-2">
                                @if ($discount->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $discount->id }})"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-100 border-l border-zinc-100 px-4 py-2">
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.discounts.edit', $discount->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary', 'disabled' => count($selectedIds) > 0],
                                    ['wireClick' => 'confirmDelete(' . $discount->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => count($selectedIds) > 0],
                                ]" />
                            </td>

                        </tr>
                        @if ($viewingId === $discount->id)
                            <x-admin-row-details colspan="12">
                                <x-admin-row-details.item label="Name">{{ $discount->name }}</x-admin-row-details.item>
                                <x-admin-row-details.item label="Type">{{ $discount->type === 'percentage' ? 'Percentage' : 'Fixed Amount' }}</x-admin-row-details.item>
                                <x-admin-row-details.item label="Value">
                                    {{ $discount->type === 'percentage'
                                        ? rtrim(rtrim(number_format((float) $discount->value, 2), '0'), '.').'% off'
                                        : \App\Models\Setting::get('currency_symbol', '৳').number_format((float) $discount->value, 2).' off' }}
                                </x-admin-row-details.item>
                                <x-admin-row-details.item label="Starts">{{ $discount->starts_at?->toDisplay() ?: '—' }}</x-admin-row-details.item>
                                <x-admin-row-details.item label="Ends">{{ $discount->ends_at?->toDisplay() ?: 'Never' }}</x-admin-row-details.item>
                                <x-admin-row-details.item label="Products">
                                    @if ($discount->products_count > 0)
                                ...
                                <x-admin-row-details.item label="Created by">{{ $discount->creator?->name ?? '—' }}</x-admin-row-details.item>
                            </x-admin-row-details>
                        @endif
                    @empty
                        <tr>
                            <td colspan="12" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm text-zinc-600">No discounts found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="discount-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'discount-delete') $flux.modal('discount-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'discount-delete') $flux.modal('discount-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete discount?</flux:heading>
            </div>
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="discount-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'discount-bulk-delete') $flux.modal('discount-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'discount-bulk-delete') $flux.modal('discount-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ Str::plural('discount', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone.</flux:text>
            <div class="flex gap-2 pt-1">
