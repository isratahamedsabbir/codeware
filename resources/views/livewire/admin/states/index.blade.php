@push('page-header-actions')
    <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.states.create') }}" wire:navigate>
        New state
    </flux:button>
@endpush
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($states as $state)
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors" @contextmenu.prevent="$el.querySelector('[data-actions-trigger]')?.click()">
                            ...
                            {{-- Divisions --}}
                            <td class="px-4 py-2 text-sm text-zinc-700">
                                {{ $state->divisions_count }}
                            </td>

                            {{-- Status --}}
                            <td class="px-4 py-2">
                                @if ($state->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $state->id }})"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-50/30 border-l border-zinc-100 px-4 py-2">
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.states.edit', $state->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary'],
                                    ['wireClick' => 'confirmDelete(' . $state->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500'],
                                ]" />
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm text-zinc-600">No states found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="state-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'state-delete') $flux.modal('state-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'state-delete') $flux.modal('state-delete').close()">
        <div class="space-y-4">
