...
<div>
    ...
    <div class="mb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
            @include('partials.admin-breadcrumbs', ['routeName' => 'admin.divisions'])
        </div>
        <div class="flex items-center gap-2 shrink-0">
            @if (count($selectedIds) > 0)
                ...
                <flux:button variant="outline" size="sm" icon="arrow-down-tray"
                    href="{{ route('admin.divisions.export', ['ids' => $selectedIds]) }}">
            ...
            <div class="page-header-actions flex items-center gap-2 shrink-0">
                <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.divisions.create') }}" wire:navigate>
                    New division
                </flux:button>
            </div>
        </div>
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
    ...
    <div class="flex items-center gap-3 p-4 flex-wrap">
        ...
        <div class="relative max-w-xs ml-auto">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search divisions…"
            ...
            <table class="w-full divide-y divide-gray-200" style="table-layout:fixed">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($divisions as $division)
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors cursor-default {{ in_array($division->id, $selectedIds, true) ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-300' : '' }}"
                            @contextmenu.prevent="$el.querySelector('[data-actions-trigger]')?.click()"
                            @click="if ($event.ctrlKey || $event.metaKey) { $event.preventDefault(); $wire.toggleSelect({{ $division->id }}) }">
                            ...
                            <td class="px-2 py-2 text-center text-xs text-zinc-500">
                                <div class="flex items-center justify-center gap-1.5" @click.stop>
                                    @if (count($selectedIds) > 0)
                                        <input type="checkbox" wire:click="toggleSelect({{ $division->id }})"
                                            @checked(in_array($division->id, $selectedIds, true))
                                            class="size-4 rounded border-zinc-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
                                    @endif
                                    <x-copy-text :text="$division->id" class="text-xs text-zinc-500">{{ $division->id }}</x-copy-text>
                                </div>
                            </td>
                            ...
                            {{-- Districts --}}
                            <td class="px-4 py-2 text-sm text-zinc-700">
                                {{ $division->districts_count }}
                            </td>

                            {{-- Status --}}
                            <td class="px-4 py-2">
                                @if ($division->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $division->id }})"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-50/30 border-l border-zinc-100 px-4 py-2">
                                ...
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.divisions.edit', $division->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary', 'disabled' => $bulkActive],
                                    ['wireClick' => 'confirmDelete(' . $division->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => $bulkActive],
                                ]" />
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm text-zinc-600">No divisions found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="division-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'division-delete') $flux.modal('division-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'division-delete') $flux.modal('division-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete division?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This will also permanently delete all districts and upazilas under
                this division.
            </flux:text>
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="division-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'division-bulk-delete') $flux.modal('division-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'division-bulk-delete') $flux.modal('division-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ \Illuminate\Support\Str::plural('division', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This will also permanently delete all districts and upazilas under
                these divisions.
            </flux:text>
