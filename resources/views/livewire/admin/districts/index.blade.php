        ...
        <div class="flex items-center gap-2 shrink-0">
            @if (count($selectedIds) > 0)
                ...
                <flux:button variant="outline" size="sm" icon="arrow-down-tray"
                    href="{{ route('admin.districts.export', ['ids' => $selectedIds]) }}">
            ...
            <div class="page-header-actions flex items-center gap-2 shrink-0">
                <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.districts.create') }}" wire:navigate>
                    New district
                </flux:button>
            </div>
        </div>
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
    ...
    <div class="flex items-center gap-3 p-4 flex-wrap">
        ...
        <div class="relative max-w-xs ml-auto">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search districts…"
            ...
            <table class="w-full divide-y divide-gray-200" style="table-layout:fixed">
                ...
                <thead>
                    <tr class="bg-zinc-50">
                        ...
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Division</th>
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Upazilas</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($districts as $district)
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors cursor-default {{ in_array($district->id, $selectedIds, true) ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-300' : '' }}"
                            @contextmenu.prevent="$el.querySelector('[data-actions-trigger]')?.click()"
                            @click="if ($event.ctrlKey || $event.metaKey) { $event.preventDefault(); $wire.toggleSelect({{ $district->id }}) }">
                            ...
                            <td class="px-2 py-2 text-center" @click.stop>
                                @if (count($selectedIds) > 0)
                                    <input type="checkbox" wire:click="toggleSelect({{ $district->id }})"
                                        @checked(in_array($district->id, $selectedIds, true))
                            ...
                            <td class="px-2 py-2 text-center text-xs text-zinc-500">
                                <x-copy-text :text="$district->id" class="text-xs text-zinc-500">{{ $district->id }}</x-copy-text>
                            </td>

                            {{-- Name --}}
                            <td class="px-4 py-2">
                                <div class="font-medium text-zinc-900 text-sm leading-snug">
                                    <x-truncate :text="$district->name" />
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-2 text-sm text-zinc-700">
                                {{ $district->upazilas_count }}
                            </td>

                            {{-- Status --}}
                            <td class="px-4 py-2">
                                @if ($district->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $district->id }})"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-100 border-l border-zinc-100 px-4 py-2">
                                ...
                                <x-admin-row-actions :actions="[
                                    ...
                                    ['wireClick' => 'confirmDelete(' . $district->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => $bulkActive],
                                ]" />
                            </td>
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-16 text-center">
                                <svg class="w-10 h-10 text-zinc-200 mx-auto mb-3" viewBox="0 0 24 24" fill="none"
                                    ...
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 20l-5.447-2.724A1 1 0 0 1 3 16.382V5.618a1 1 0 0 1 1.447-.894L9 7m0 13l6-3m-6 3V7m6 10l5.447 2.724A1 1 0 0 0 21 18.83V7.618a1 1 0 0 0-.553-.894L15 4m0 13V4m0 0L9 7" />
                                </svg>
                                <p class="text-sm text-zinc-600">No districts found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="district-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'district-delete') $flux.modal('district-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'district-delete') $flux.modal('district-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete district?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This will also permanently delete all upazilas under this
                district.
            </flux:text>
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="district-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'district-bulk-delete') $flux.modal('district-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'district-bulk-delete') $flux.modal('district-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ \Illuminate\Support\Str::plural('district', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This will also permanently delete all upazilas under these
                districts.
            </flux:text>
            <div class="flex gap-2 pt-1">
