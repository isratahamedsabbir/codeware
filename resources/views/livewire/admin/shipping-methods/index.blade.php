...
<div>
    ...
    <div class="mb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
            @include('partials.admin-breadcrumbs', ['routeName' => 'admin.shipping-methods'])
        </div>
        <div class="flex items-center gap-2 shrink-0">
            @if (count($selectedIds) > 0)
                ...
                <flux:button variant="outline" size="sm" icon="arrow-down-tray"
                    href="{{ route('admin.shipping-methods.export', ['ids' => $selectedIds]) }}">
            ...
            <div class="page-header-actions flex items-center gap-2 shrink-0">
                <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.shipping-methods.create') }}" wire:navigate>
                    New shipping method
                </flux:button>
            </div>
        </div>
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
    ...
    <div class="flex items-center gap-3 p-4 flex-wrap">
        ...
        <div class="relative max-w-xs ml-auto">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search shipping methods…"
                        ...
                        <tr class="group/row hover:bg-indigo-50/30 transition-colors {{ in_array($method->id, $selectedIds, true) ? 'bg-indigo-50/50' : '' }}"
                            ...
                            <td class="px-4 py-2">
                                <div class="font-medium text-zinc-900 text-sm leading-snug">
                                    <x-truncate :text="$method->name" />
                                </div>
                            </td>

                            {{-- Cost --}}
                            <td class="px-4 py-2 text-sm text-zinc-700">
                                {{ \App\Models\Setting::get('currency_symbol', '৳') }}{{ number_format((float) $method->cost, 2) }}
                            </td>

                            {{-- Status --}}
                            <td class="px-4 py-2">
                                @if ($method->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $method->id }})"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-100 border-l border-zinc-100 px-4 py-2">
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.shipping-methods.edit', $method->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary', 'disabled' => count($selectedIds) > 0],
                                    ['wireClick' => 'confirmDelete(' . $method->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => count($selectedIds) > 0],
                                ]" />
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm text-zinc-600">No shipping methods found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="shipping-method-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'shipping-method-delete') $flux.modal('shipping-method-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'shipping-method-delete') $flux.modal('shipping-method-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete shipping method?</flux:heading>
            </div>
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="shipping-method-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'shipping-method-bulk-delete') $flux.modal('shipping-method-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'shipping-method-bulk-delete') $flux.modal('shipping-method-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ Str::plural('shipping method', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone.</flux:text>
            <div class="flex gap-2 pt-1">
