<div class="bg-white rounded-lg border border-zinc-100 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between gap-3 px-6 py-5 border-b border-zinc-100 flex-wrap">
        <div>
            <h1 class="text-lg font-semibold text-zinc-900">Testimonials</h1>
            <p class="text-sm text-zinc-600 mt-0.5">Manage customer testimonials and reviews. Drag to reorder.</p>
        </div>
    ...
    <div class="flex gap-3 px-6 py-3 border-b border-zinc-100 flex-wrap">
        <div class="relative flex-1 min-w-[180px]">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search by name or comment…"
        ...
        x-data="{
            init() {
                ...
                new Sortable(this.$refs.sortableRows, {
                    ...
                    onEnd: (evt) => {
                        const rows = [...this.$refs.sortableRows.querySelectorAll('[data-testimonial-id]')];
                        const order = rows.map(r => parseInt(r.dataset.testimonialId));
                        $wire.reorder(order);
                    }
                });
            }
            ...
            <table class="w-full border-collapse" style="table-layout:fixed">
                ...
                <thead>
                    <tr class="bg-zinc-50 border-b border-zinc-100">
                        ...
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Location</th>
                ...
                <tbody x-ref="sortableRows">
                    @forelse ($testimonials as $testimonial)
                        <tr class="border-b border-zinc-50 hover:bg-indigo-50/30 transition-colors" data-testimonial-id="{{ $testimonial->id }}">
                            ...
                            <td class="px-4 py-3.5">
                                @if ($testimonial->image)
                                    <img src="{{ $testimonial->image }}" alt=""
                                        class="w-8 h-8 rounded-full object-cover border border-zinc-100" />
                                @else
                                    <div class="w-8 h-8 rounded-full bg-zinc-100 flex items-center justify-center text-zinc-400">
                                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            ...
                            <td class="px-4 py-3.5">
                                <div class="font-medium text-zinc-900 text-sm leading-snug">
                                    {{ $testimonial->getTranslation('name', 'en', false) }}
                                </div>
                                @if ($testimonial->getTranslation('name', 'bn', false))
                                    <div class="text-xs text-zinc-600 mt-0.5">
                                        {{ $testimonial->getTranslation('name', 'bn', false) }}
                                    </div>
                                @endif
                            </td>
                            ...
                            {{-- Comment --}}
                            <td class="px-4 py-3.5">
                                <p class="text-sm text-zinc-600 truncate">{{ Str::limit(strip_tags($testimonial->getTranslation('comment', 'en', false)), 80) }}</p>
                            </td>

                            {{-- Location --}}
                            <td class="px-4 py-3.5">
                                <span class="text-sm text-zinc-600">{{ $testimonial->location ?? '—' }}</span>
                            </td>
                            ...
                            <td class="px-4 py-3.5">
                                <div class="flex items-center justify-end gap-1.5">
                                    ...
                                    <div class="relative group">
                                        <a href="{{ route('admin.testimonials.edit', $testimonial->id) }}" wire:navigate
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                ...
                                <p class="text-sm text-zinc-600">No testimonials found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="testimonial-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'testimonial-delete') $flux.modal('testimonial-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'testimonial-delete') $flux.modal('testimonial-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete testimonial?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The testimonial will be soft-deleted.
            </flux:text>
