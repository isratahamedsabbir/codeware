...
<div>
    ...
    <div class="mb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
            @include('partials.admin-breadcrumbs', ['routeName' => 'admin.portfolio-skills'])
        </div>
        <div class="flex items-center gap-2 shrink-0">
            ...
            <div class="page-header-actions flex items-center gap-2 shrink-0">
                <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.portfolio-skills.create') }}" wire:navigate>
                    New skill
                </flux:button>
            </div>
        </div>
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
    ...
    <div class="flex items-center gap-3 p-4 flex-wrap">
        ...
        <div class="relative max-w-xs ml-auto">
            ...
            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search skills or groups…"
    ...
    {{-- Table — rows are draggable to set the order the theme prints them in,
         within and across groups (the group order follows each group's first row). --}}
        ...
        x-data="{
            init() {
                ...
                new Sortable(this.$refs.sortableRows, {
                    ...
                    onEnd: () => {
                        const rows = [...this.$refs.sortableRows.querySelectorAll('[data-skill-id]')];
                        $wire.reorder(rows.map(r => parseInt(r.dataset.skillId)));
                    }
                });
            }
        }">
            ...
            <table class="w-full divide-y divide-gray-200" style="table-layout:fixed">
                ...
                <thead>
                    <tr class="bg-zinc-50">
                        ...
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Group</th>
                ...
                <tbody x-ref="sortableRows" class="divide-y divide-gray-200">
                    @forelse ($skills as $skill)
                        <tr wire:key="skill-{{ $skill->id }}" data-skill-id="{{ $skill->id }}"
                            class="group/row hover:bg-indigo-50/30 transition-colors cursor-default {{ in_array($skill->id, $selectedIds, true) ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-300' : '' }}"
                            @contextmenu.prevent="$el.querySelector('[data-actions-trigger]')?.click()"
                            @click="if ($event.ctrlKey || $event.metaKey) { $event.preventDefault(); $wire.toggleSelect({{ $skill->id }}) }">

                            {{-- Bulk-select checkbox --}}
                            <td class="px-2 py-2 text-center" @click.stop>
                                @if (count($selectedIds) > 0)
                                    <input type="checkbox" wire:click="toggleSelect({{ $skill->id }})"
                                        @checked(in_array($skill->id, $selectedIds, true))
                            ...
                            <td class="px-2 py-2 text-center text-xs text-zinc-500">
                                <x-copy-text :text="$skill->id" class="text-xs text-zinc-500">{{ $skill->id }}</x-copy-text>
                            </td>

                            {{-- Icon (emoji) --}}
                            <td class="px-4 py-2 text-base">{{ $skill->icon ?: '—' }}</td>

                            {{-- Name --}}
                            <td class="px-4 py-2">
                                <div class="font-medium text-zinc-900 text-sm leading-snug"
                                    @if ($skill->getTranslation('name', 'bn', false))
                                        title="{{ $skill->getTranslation('name', 'en', false) }} — {{ $skill->getTranslation('name', 'bn', false) }}"
                                    @endif>
                                    <x-truncate :text="$skill->getTranslation('name', 'en', false)" />
                                </div>
                            </td>

                            {{-- Group (the free-text column the theme builds its skill-group columns from) --}}
                            <td class="px-4 py-2">
                                <span class="px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 text-xs font-medium">{{ $skill->group }}</span>
                            </td>

                            {{-- Status --}}
                            <td class="px-4 py-2">
                                @if ($skill->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $skill->id }})"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-100 border-l border-zinc-100 px-4 py-2">
                                ...
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.portfolio-skills.edit', $skill->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary', 'disabled' => $bulkActive],
                                    ['wireClick' => 'confirmDelete(' . $skill->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => $bulkActive],
                                ]" />
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-16 text-center">
                                <flux:icon.sparkles class="w-10 h-10 text-zinc-200 mx-auto mb-3" />
                                <p class="text-sm text-zinc-600">No skills found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="skill-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'skill-delete') $flux.modal('skill-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'skill-delete') $flux.modal('skill-delete').close()">
        <div class="space-y-4">
            ...
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The skill will be soft-deleted.
            </flux:text>
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="skill-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'skill-bulk-delete') $flux.modal('skill-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'skill-bulk-delete') $flux.modal('skill-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ \Illuminate\Support\Str::plural('skill', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The selected skills will be soft-deleted.
            </flux:text>
            <div class="flex gap-2 pt-1">
