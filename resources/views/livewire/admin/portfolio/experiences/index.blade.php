...
<div>
    ...
    <div class="mb-3 flex items-center justify-between gap-4 flex-wrap">
        <div>
            @include('partials.admin-breadcrumbs', ['routeName' => 'admin.portfolio-experiences'])
        </div>
        <div class="flex items-center gap-2 shrink-0">
            ...
            <div class="page-header-actions flex items-center gap-2 shrink-0">
                <flux:button variant="ghost" size="sm" icon="plus" href="{{ route('admin.portfolio-experiences.create') }}" wire:navigate>
                    New entry
                </flux:button>
            </div>
        </div>
...
<div class="bg-white rounded-[5px] shadow-sm overflow-hidden">
    ...
    {{-- Table — rows are draggable to set the order the theme prints the timeline in --}}
        ...
        x-data="{
            init() {
                ...
                new Sortable(this.$refs.sortableRows, {
                    ...
                    onEnd: () => {
                        const rows = [...this.$refs.sortableRows.querySelectorAll('[data-experience-id]')];
                        $wire.reorder(rows.map(r => parseInt(r.dataset.experienceId)));
                    }
                });
            }
        }">
            ...
            <table class="w-full divide-y divide-gray-200" style="table-layout:fixed">
                ...
                <thead>
                    <tr class="bg-zinc-50">
                        ...
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Company</th>
                        <th class="px-4 py-2.5 text-left text-[10.5px] font-semibold text-zinc-600 uppercase tracking-wider">Period</th>
                ...
                <tbody x-ref="sortableRows" class="divide-y divide-gray-200">
                    @forelse ($experiences as $experience)
                        <tr wire:key="experience-{{ $experience->id }}" data-experience-id="{{ $experience->id }}"
                            class="group/row hover:bg-indigo-50/30 transition-colors cursor-default {{ in_array($experience->id, $selectedIds, true) ? 'bg-indigo-50 ring-1 ring-inset ring-indigo-300' : '' }}"
                            @contextmenu.prevent="$el.querySelector('[data-actions-trigger]')?.click()"
                            @click="if ($event.ctrlKey || $event.metaKey) { $event.preventDefault(); $wire.toggleSelect({{ $experience->id }}) }">

                            {{-- Bulk-select checkbox --}}
                            <td class="px-2 py-2 text-center" @click.stop>
                                @if (count($selectedIds) > 0)
                                    <input type="checkbox" wire:click="toggleSelect({{ $experience->id }})"
                                        @checked(in_array($experience->id, $selectedIds, true))
                            ...
                            <td class="px-2 py-2 text-center text-xs text-zinc-500">
                                <x-copy-text :text="$experience->id" class="text-xs text-zinc-500">{{ $experience->id }}</x-copy-text>
                            </td>

                            {{-- Role --}}
                            <td class="px-4 py-2">
                                <div class="font-medium text-zinc-900 text-sm leading-snug"
                                    @if ($experience->getTranslation('role', 'bn', false))
                                        title="{{ $experience->getTranslation('role', 'en', false) }} — {{ $experience->getTranslation('role', 'bn', false) }}"
                                    @endif>
                                    <x-truncate :text="$experience->getTranslation('role', 'en', false)" />
                                </div>
                            </td>

                            {{-- Company --}}
                            <td class="px-4 py-2 text-sm text-zinc-700">
                                <x-truncate :text="$experience->getTranslation('company', 'en', false) ?: '—'" />
                            </td>

                            {{-- Period --}}
                            <td class="px-4 py-2 text-sm text-zinc-700">
                                <x-truncate :text="$experience->period ?: '—'" />
                            </td>

                            {{-- Status --}}
                            <td class="px-4 py-2">
                                @if ($experience->status === 'active')
                                    <button type="button" wire:click="toggleStatus({{ $experience->id }})"
                            ...
                            <td class="sticky right-0 z-10 bg-white group-hover/row:bg-indigo-100 border-l border-zinc-100 px-4 py-2">
                                ...
                                <x-admin-row-actions :actions="[
                                    ['href' => route('admin.portfolio-experiences.edit', $experience->id), 'icon' => 'pencil', 'label' => 'Edit', 'color' => 'primary', 'disabled' => $bulkActive],
                                    ['wireClick' => 'confirmDelete(' . $experience->id . ')', 'icon' => 'trash', 'label' => 'Delete', 'color' => 'rose-500', 'disabled' => $bulkActive],
                                ]" />
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                <flux:icon.clock class="w-10 h-10 text-zinc-200 mx-auto mb-3" />
                                <p class="text-sm text-zinc-600">No experience entries found.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    {{-- Delete Modal --}}
    <flux:modal name="experience-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'experience-delete') $flux.modal('experience-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'experience-delete') $flux.modal('experience-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                ...
                <flux:heading>Delete experience entry?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The entry will be soft-deleted.
            </flux:text>
    ...
    {{-- Bulk Delete Modal --}}
    <flux:modal name="experience-bulk-delete" class="md:w-80"
        x-on:open-modal.window="if ($event.detail.name === 'experience-bulk-delete') $flux.modal('experience-bulk-delete').show()"
        x-on:close-modal.window="if ($event.detail.name === 'experience-bulk-delete') $flux.modal('experience-bulk-delete').close()">
        <div class="space-y-4">
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-red-50 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 text-red-500" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <flux:heading>Delete {{ count($selectedIds) }} {{ \Illuminate\Support\Str::plural('entry', count($selectedIds)) }}?</flux:heading>
            </div>
            <flux:text class="text-sm text-zinc-500">This action cannot be undone. The selected entries will be soft-deleted.
            </flux:text>
            <div class="flex gap-2 pt-1">
